...
class RSA_OAEP extends RSA {
    ...
    encryptData(data) {
        const key_size = Math.ceil(this.key[0].bitLength / 8);
        ...
        for (let i = 0; i < seed.length; i++)
            seed[i] ^= seed_mask[i];
        this.encrypt(new BigNumber(message));
        return this;
    }
    decryptData(data) {
        if (!this.key)
        ...
        this.decrypt(new BigNumber(data));
        const z = this.result[0];
        const seed = this.result.subarray(1, hash_size + 1);
        const data_block = this.result.subarray(hash_size + 1);
        if (z !== 0)
        ...
        for (let i = 0; i < data_block.length; i++)
            data_block[i] ^= data_block_mask[i];
        const lhash = this.hash.process(this.label || new Uint8Array(0)).finish().result;
...
class RSA_PSS extends RSA {
    constructor(key, hash, saltLength = 4) {
        ...
        if (this.key !== null && Math.ceil((this.key[0].bitLength - 1) / 8) < this.hash.HASH_SIZE + this.saltLength + 2)
            throw new SyntaxError('saltLength is too large');
    }
    sign(data) {
        const key_bits = this.key[0].bitLength;
        const hash_size = this.hash.HASH_SIZE;
        ...
        this.decrypt(new BigNumber(message));
        return this;
    }
    verify(signature, data) {
        ...
        const salt_length = this.saltLength;
        const ps_length = message_length - salt_length - hash_size - 2;
        this.encrypt(new BigNumber(signature));
...
class RSA_PKCS1_v1_5 extends RSA {
    ...
    sign(data) {
        ...
        const k = (this.key[0].bitLength + 7) >> 3;
        if (k < t_len + 11) {
    ...
    verify(signature, data) {
        ...
        let res = 1;
        // EM = 0x00 || 0x01 || PS || 0x00 || T
        const decryptedSignature = this.result;
        let i = 0;
...
export { string_to_bytes, hex_to_bytes, base64_to_bytes, bytes_to_string, bytes_to_hex, bytes_to_base64, IllegalStateError, IllegalArgumentError, SecurityError, AES_CBC, AES_CFB, AES_CMAC, AES_CTR, AES_ECB, AES_GCM, AES_OFB, BigNumber, Modulus, Sha1, Sha256, Sha512, HmacSha1, HmacSha256, HmacSha512, Pbkdf2HmacSha1, Pbkdf2HmacSha256, Pbkdf2HmacSha512, RSA_OAEP, RSA_PKCS1_v1_5, RSA_PSS, RSA };
