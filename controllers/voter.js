        ...
        VoterModel.findOne({email:req.body.email,election_address:req.body.election_address}, function(err, result) {
            ...
            if(err){
                ...
                if(!result){
                    ...
                    VoterModel.create({ email: req.body.email, password: req.body.email, election_address: req.body.election_address }, function (err, voter) {
                        ...
                        if (err) 
                            ...
                            const mailOptions = {
                                ...
                                html: req.body.election_description+'<br>Your voting id is:'+voter.email+'<br>'+'Your password is:'+voter.password+'<br><a href="url">Click here to visit the website</a>'// plain text body
        ...
        VoterModel.findOne({email:req.body.email,password:req.body.password}, function(err, voterInfo){
            ...
            if (err) 
                ...
                if(voterInfo)
                    ...
                    res.json({status:"success", message: "voter found!!!", data:{id: voterInfo._id, election_address: voterInfo.election_address}});
    ...
    getAll: function(req, res, cb) {
        ...
        VoterModel.find({election_address:req.body.election_address}, function(err, voters) {
            ...
            if (err)
                ...
                res.json({status:"success", message: "voters list found!!!", data:{voters: voterList},count:count});
    ...
    updateById: function(req, res, cb) {
        ...
        VoterModel.findOne({email:req.body.email}, function(err, result) {
            if(err){
                ...
                VoterModel.findByIdAndUpdate(req.params.voterId,{email:req.body.email,password: password}, function(err, voter){
            if(err)
                cb(err);
        });
        ...
        VoterModel.findById(req.params.voterId, function(err, voterInfo) {
            ...
            const mailOptions = {
                ...
                html: req.body.election_description+'<br>Your voting id is:'+voterInfo.email+'<br>'+'Your password is:'+voterInfo.password+'<br><a href="url">Click here to visit the website</a>'// plain text body
                ...
                for (let voter of voters) {
                    ...
                    var transporter = nodemailer.createTransport({
                        ...
                        auth: {
                            ...
                            pass: '' /*sender email ID's password */
